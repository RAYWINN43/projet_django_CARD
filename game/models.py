...
class Deck:
    ...
    def from_dict(cls, payload):
        ...
        deck = cls(visible=visible, visible_first=visible_first)
    ...
    def __str__(self):
        ...
        for i, card in enumerate(self.cards):
            to_display += (
                str(card) if self.visible or (self.visible_first and i == 0) else "XX"
            )
            to_display += ", "
        return to_display[:-2]
...
class Game(models.Model):
    ...
    def hit(self):
        if self._running:
            ...
            self.save()
            return self._player_hand.hand_value() > 21
    ...
    def check_results(self):
        if self._running:
            ...
            if (player_score < croupier_score <= 21) or player_score > 21:
                log += "\nLe croupier a gagné"
