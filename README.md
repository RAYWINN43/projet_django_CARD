...
| --- | --- |
| Evan MARTIN | UI/UX, maquettes, responsive et sound design |
| Antoine BLAIN | authentification, infrastructure Docker/Redis/PostgreSQL et CI |
| Corentin PECONTAL | backend Django, modèles et logique de jeu |

## Auto-évaluation

| Domaine de la grille | Estimation | Justification |
| --- | ---: | --- |
| Backend Django | **4,5 / 6** | L’authentification Django est fonctionnelle, les utilisateurs possèdent un profil joueur et les jetons sont stockés en base. La logique principale du BlackJack est présente avec la distribution, les actions du joueur et le calcul du résultat.l’historique complet des coups reste à améliorer. |
| UI/UX | **4 / 5** | L’interface propose les écrans principaux : accueil, connexion, inscription, choix d’avatar et jeu. Les styles utilisent des variables CSS et une logique proche de l’Atomic Design. Des améliorations restent possibles sur l’accessibilité, les animations avancées et certains retours visuels pendant la partie. |
| Infrastructure | **4,5 / 5** | Le projet utilise Docker Compose avec les services Django, PostgreSQL et Redis. Les services sont interconnectés avec un réseau Docker, les volumes nommés sont présents, Django tourne avec Gunicorn, les fichiers statiques sont gérés et les healthchecks sont configurés. |
| Tests, qualité et CI | **3 / 4** | Black et Ruff sont configurés, et la CI GitHub Actions vérifie le formatage, le linting et le check Django. Les tests automatisés peuvent encore être renforcés sur la logique complète du jeu, les permissions et les transitions d’état. |
